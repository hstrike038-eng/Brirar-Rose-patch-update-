(() => {
  ...
  const pickups = [
    { x: 6.5, y: 5.5, type: "poison", active: true },
    { x: 15.4, y: 8.5, type: "poison", active: true },
    { x: 5.5, y: 13.5, type: "poison", active: true },
    { x: 11.5, y: 17.5, type: "poison", active: true },
  ];
  ...
  function unstuckBody(body) {
    if (canStand(body.x, body.y)) return;
    const offsets = [
      [0.26, 0],
      [-0.26, 0],
      [0, 0.26],
    ...
    for (const [ox, oy] of offsets) {
      if (canStand(body.x + ox, body.y + oy)) {
        body.x += ox;
        body.y += oy;
        return;
      }
    }
    body.x += (Math.random() - 0.5) * 0.4;
    body.y += (Math.random() - 0.5) * 0.4;
  }
  ...
  function damagePlayer(amount) {
    ...
    if (player.poisonImmuneTimer > 0) {
      showMessage("Poison immunity active", 1.2);
  ...
  function update(dt) {
    ...
    if (player.poisonImmuneTimer > 0) player.poisonImmuneTimer -= dt;
  ...
  function updatePickups() {
    for (const pickup of pickups) {
      ...
      pickup.active = false;
        if (pickup.type === "poison") {
        player.health = Math.min(100, player.health + 32);
        player.healTimer = 0.9;
        player.poisonImmuneTimer = 18;
        showMessage("Toxin serum secured", 1.6);
  ...
  function updateEnemies(dt) {
    for (const enemy of enemies) {
      ...
      if (seesPlayer || dist < 4.8) enemy.alert = true;
      if (!enemy.alert) {
        enemy.phase += dt;
        const driftX = Math.cos(enemy.phase * 0.7) * 0.25 * dt;
        const driftY = Math.sin(enemy.phase * 0.9) * 0.25 * dt;
        moveBody(enemy, driftX, driftY);
        continue;
      }

      if (dist > 1.02) {
        const speed = (enemy.hurt > 0 ? 0.25 : 0.55) * dt;
  ...
  function drawPickup(pickup, x, y, size) {
    ...
    btx.globalAlpha = 0.9;
    if (pickup.type === "poison") {
      btx.fillStyle = "#8b3eb8";
      btx.fillRect(x + size * 0.26, y + size * 0.18, size * 0.48, size * 0.58);
      btx.fillRect(x + size * 0.38, y + size * 0.08, size * 0.24, size * 0.14);
      btx.fillStyle = "#c2ff5a";
      btx.fillRect(x + size * 0.32, y + size * 0.42, size * 0.36, size * 0.16);
    } else {
  ...
  function drawMinimap() {
    ...
    for (const pickup of pickups) {
      if (!pickup.active) continue;
      btx.fillStyle = pickup.type === "poison" ? "#8b3eb8" : "#f6c945";
  ...
  function syncHud() {
    ...
    if (player.poisonImmuneTimer > 0) {
      healthFillEl.style.background =
        "repeating-linear-gradient(90deg, rgba(255, 255, 255, 0.16) 0 2px, transparent 2px 8px), linear-gradient(90deg, #85d191, #3d9c5d)";
      healthFillEl.classList.add("is-immune");
    } else {
      healthFillEl.style.background =
        player.health > 55
      ...
      healthFillEl.classList.remove("is-immune");
    }
